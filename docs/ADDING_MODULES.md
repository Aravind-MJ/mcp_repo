...
3. Declare the MCP access policy explicitly. Mount `sharedSecretAuth(config.secretFile)` before `/<module>/mcp` unless the module is intentionally public.
...
7. Add the module name to `/healthz` only after its dependency checks pass. Update the watchdog only when that module is required for the hub process to count as healthy; optional integrations must not restart-loop the core service while unconfigured.
